...
export function SuggistionPinPlaceHolder(props) {
    let colorSlotsCount = props.suggistion.length
    let style = {gridTemplateColumns: `repeat(${colorSlotsCount}, ${60/colorSlotsCount}%)`};



    let pins = props.suggistion.map((color, index) => <div  key={index} className='pin-place-holder'><Pin color={color} /> </div>)
